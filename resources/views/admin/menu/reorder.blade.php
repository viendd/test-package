...
@section('content')
    ...
    ?>
    <div class="row mt-4">
        <form action="{{route('menu.reorder')}}" class="form_select_language" id="form_select_language">
            <select name="language_id" id="language_id" class="form-control">
                @foreach($languages as $language)
                    <option value="{{$language->id}}" {{!request()->language_id && $language->id == \App\Models\Language::VI ? 'selected' : ''}} {{request()->language_id == $language->id ? 'selected' : ''}}>{{$language->name}}</option>
                @endforeach
            </select>
        </form>
    </div>
